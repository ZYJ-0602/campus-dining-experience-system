...
默认账号：`admin` / `123456`，`operator` / `123456`。

### 真实演示数据补种

如果你想让首页和审核页立刻显示更真实的中文内容，可以执行：

```bash
python seed_real_demo_data.py
```

这个脚本是幂等的，默认只处理校区1，不会重建数据库。它会补入真实作者、真实笔记、敏感词和本地图片引用；如果发现明显乱码的演示记录，也会做定向修复。

## 🧪 测试

在仓库根目录执行 `pytest`，会自动收集 `dining_system/tests` 下的正式测试用例。
